...
impl SampleArgs {
    pub fn run(&self) -> Result<()> {
        let (config, order) = load_dataset_config(&self.config)?;
        ...
        let root_glob = self.parquet_glob_pattern(&root.name);
        let total_rows: u64 = conn
            .query_row(
                &format!("SELECT count(*) FROM read_parquet('{root_glob}')"),
                [],
                |row| row.get(0),
            )
            .with_context(|| format!("Failed to count rows for '{root_glob}'"))?;

        if total_rows == 0 {
            bail!("Root table '{}' has no rows", root.name);
        }

        let target = self.rows;
        ...
        for &idx in &order {
            ...
            let glob = self.parquet_glob_pattern(&table.name);
    ...
    fn parquet_glob_pattern(&self, table_name: &str) -> String {
        format!(
            "{}/{table_name}/*.parquet",
            self.input.trim_end_matches('/')
        )
    }
}
